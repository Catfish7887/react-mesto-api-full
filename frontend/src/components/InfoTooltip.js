function InfoTooltip({ state, onClose, isOpened }) {
  const authSuccess = (<>
    ...
    <span className='popup__name'>Вы успешно зарегистрировались</span>
  </>)
  ...
  const authFailed = (<>
    ...
    <span className='popup__name'>Что-то пошло не так</span>
  </>)

  return (
    <div className={`popup popup_type_tooltip ${isOpened ? "popup_opened" : 'popup_closed'}`}
      onMouseDown={(e) => { e.target.classList.contains('popup_opened') && onClose() }}
    >
      <div className="popup__container popup__container_type_tooltip">
        {state ? authSuccess : authFailed}
